...
import { ArrowLeft, Music2, Sparkles, ListMusic } from 'lucide-react';
...
function DayDetail() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      setSavedJournalText(journalData?.entry_text || '');
      setMood(moodData);
  ...
  const handleSaveJournal = async () => {
    ...
    try {
      const response = await fetch(`/api/journal/${date}`, {
        ...
        body: JSON.stringify({ entry_text: journalText }),
      });
      if (!response.ok) {
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center gap-4 mt-3">
            <button
              ...
              disabled={isSaving || !journalText.trim() || journalText === savedJournalText}
          ...
          <div>
            ...
            <ul className="space-y-2">
              {recommendations.map((rec) => (
                <li
                  key={rec.id}
                  className={`${theme.buttonBg} rounded-xl p-4 flex flex-col gap-1`}
                >
                  {rec.spotify_url ? (
                    <a
                      ...
                      className={`font-semibold hover:text-${theme.primaryAccent} transition-colors duration-200`}
                    >
                      {rec.track_name} — {rec.artist_name}
                    </a>
                  ) : (
                    <span className="font-semibold">{rec.track_name} — {rec.artist_name}</span>
                  )}
